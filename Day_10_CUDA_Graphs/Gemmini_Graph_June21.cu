...
int main()
{
    ...
    singleGpuResults.kernel_exec_time = singleGpuResults.total_time; // For graphs, total time IS execution time
    ...
    nvtxRangePop(); // Single_GPU_Execution_And_Timing
    ...
    nvtxRangePop(); // Multi_GPU_Setup
    
    nvtxRangePush("Multi_GPU_Execution_And_Timing");
    cudaEvent_t multi_start, multi_stop;
    cudaEvent_t h2d_start, h2d_stop, exec_start, exec_stop, d2h_start, d2h_stop;
    ...
    gpuCheck(cudaEventRecord(h2d_start, 0), "record h2d start");
    ...
    gpuCheck(cudaEventRecord(h2d_stop, 0), "record h2d stop");

    gpuCheck(cudaEventRecord(exec_start, 0), "record exec start");
    ...
    gpuCheck(cudaEventRecord(exec_stop, 0), "record exec stop");

    gpuCheck(cudaEventRecord(d2h_start, 0), "record d2h start");
    ...
    gpuCheck(cudaEventRecord(d2h_stop, 0), "record d2h stop");
    ...
    nvtxRangePop(); // Multi_GPU_Execution_And_Timing
    ...
    multiLaunchTime = std::max(0.0f, multiLaunchTime); // Ensure non-negative
