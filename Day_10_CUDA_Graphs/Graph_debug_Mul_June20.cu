...
// Create a single stream for capturing and launching the graph
cudaStream_t graphStream;
gpuCheck(cudaStreamCreate(&graphStream), "create graph stream");
...
gpuCheck(cudaEventCreate(&single_start), "create single start");
gpuCheck(cudaEventCreate(&single_stop), "create single stop");
gpuCheck(cudaEventCreate(&setup_start), "create setup start");
gpuCheck(cudaEventCreate(&setup_stop), "create setup stop");
...
cudaGraph_t graph = nullptr;
cudaGraphExec_t graphExec = nullptr;
float setup_time = 0.0f;

printf("Processing ENTIRE dataset on single GPU 0 using CUDA Graph...\n");
printf("Dataset size: %.2f GB\n", (batchBytes * 3) / (1024.0f * 1024.0f * 1024.0f));

// --- CUDA Graph Setup Phase ---
printf("  Phase 1: Capturing and Instantiating CUDA Graph...\n");
gpuCheck(cudaEventRecord(setup_start), "record setup start");

// 1. Begin Capture on the graph stream
gpuCheck(cudaStreamBeginCapture(graphStream, cudaStreamCaptureModeGlobal), "begin capture");

// Record all operations for the graph: H2D -> Kernels -> D2H
// NOTE: We put all operations into the graph to measure the full, optimized pipeline.
gpuCheck(cudaMemcpyAsync(dev_X_single, batch_X, batchBytes, cudaMemcpyHostToDevice, graphStream), "capture copy X");
gpuCheck(cudaMemcpyAsync(dev_H_single, batch_H, batchBytes, cudaMemcpyHostToDevice, graphStream), "capture copy H");

// Record all kernel launches into the SAME capture stream
dim3 block(BLOCKSIZE);
int singleKernelsLaunched = 0;
int symbolsPerStreamChunk = BATCH_SIZE_SYMBOLS / STREAMS_PER_GPU; // Use this to mimic the original workload structure
int remainingSymbols = BATCH_SIZE_SYMBOLS % STREAMS_PER_GPU;

for (int s = 0; s < STREAMS_PER_GPU; s++) {
    int chunkStartSymbol = s * symbolsPerStreamChunk;
    int chunkSymbols = symbolsPerStreamChunk + (s == STREAMS_PER_GPU - 1 ? remainingSymbols : 0);
    int chunkElements = chunkSymbols * ELEMENTS_PER_SYMBOL;
    size_t chunkOffset = chunkStartSymbol * ELEMENTS_PER_SYMBOL;

    if (chunkElements > 0) {
        dim3 grid((chunkElements + BLOCKSIZE - 1) / BLOCKSIZE);
        pw_multiply_half_kernel<<<grid, block, 0, graphStream>>>(
            (ComplexType*)((char*)dev_X_single + chunkOffset),
            (ComplexType*)((char*)dev_H_single + chunkOffset),
            (ComplexType*)((char*)dev_Y_single + chunkOffset),
            chunkElements);
        singleKernelsLaunched++;
    }
}
...
// Record D2H copy
gpuCheck(cudaMemcpyAsync(batch_Y, dev_Y_single, batchBytes, cudaMemcpyDeviceToHost, graphStream), "capture copy Y");

// 2. End Capture
gpuCheck(cudaStreamEndCapture(graphStream, &graph), "end capture");

// 3. Instantiate Graph
gpuCheck(cudaGraphInstantiate(&graphExec, graph, NULL, NULL, 0), "instantiate graph");

gpuCheck(cudaEventRecord(setup_stop), "record setup stop");
gpuCheck(cudaEventSynchronize(setup_stop), "sync setup stop");
gpuCheck(cudaEventElapsedTime(&setup_time, setup_start, setup_stop), "elapsed setup time");
printf("  Graph setup complete: %.2f ms\n", setup_time);

// --- CUDA Graph Execution Phase ---
...
gpuCheck(cudaGraphLaunch(graphExec, graphStream), "launch graph");
...
gpuCheck(cudaStreamSynchronize(graphStream), "sync graph stream");
...
gpuCheck(cudaEventElapsedTime(&singleGpuResults.kernel_exec_time, exec_start, exec_stop), "elapsed exec");
gpuCheck(cudaEventElapsedTime(&singleGpuResults.total_time, single_start, single_stop), "elapsed total");

// For the breakdown table, we'll estimate transfer times from a separate run if needed,
// as they are now part of the opaque graph execution. For this purpose, we can assign
// the total time to the execution time for a simpler, yet more honest, comparison.
// Or, we can just look at the total time. Let's report the full pipeline time.
// We'll set the H2D, D2H times to 0 since they are now part of the measured `total_time`.
singleGpuResults.h2d_time = 0; // Included in total
singleGpuResults.d2h_time = 0; // Included in total
// The most important metric is the total time from launch to sync.
singleGpuResults.kernel_exec_time = singleGpuResults.total_time;


singleGpuResults.num_kernels_launched = 1; // We do ONE graph launch
singleGpuResults.num_streams_used = 1;

printf("Single GPU processing complete!\n");
printf("  CUDA Graph launches:    %d (optimized from %d individual kernel launches)\n", singleGpuResults.num_kernels_launched, singleKernelsLaunched);
printf("  Streams used:           %d (graph stream)\n", singleGpuResults.num_streams_used);
printf("  Total time:             %.2f ms\n", singleGpuResults.total_time);
