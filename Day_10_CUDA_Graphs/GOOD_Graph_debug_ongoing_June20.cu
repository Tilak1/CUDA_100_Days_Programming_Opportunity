...
int main()
{
    ...
    // Perform the launch and record events into the stream to measure GPU time
    ...
    // Wait for the main work to finish
    ...
    printf("                          -----------    --------------\n");
    printf("H2D Transfer:           %8s      %8.2f ms\n", "N/A*", multiGpuResults.h2d_time);
    printf("Kernel/Graph Launch:    %8.3f ms      %8.2f ms\n", singleGpuResults.kernel_launch_time, (multiGpuResults.total_time - multiGpuResults.h2d_time - multiGpuResults.kernel_exec_time - multiGpuResults.d2h_time));
    printf("Kernel/Graph Execution: %8.2f ms      %8.2f ms\n", singleGpuResults.kernel_exec_time, multiGpuResults.kernel_exec_time);
    printf("D2H Transfer:           %8s      %8.2f ms\n", "N/A*", multiGpuResults.d2h_time);
    printf("---------------------     -----------    --------------\n");
    printf("TOTAL TIME:             %8.2f ms      %8.2f ms\n", singleGpuResults.total_time, multiGpuResults.total_time);
