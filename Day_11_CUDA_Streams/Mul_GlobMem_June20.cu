...
int main()
{
    ...
    // Transfer results using multiple streams
    for (int s = 0; s < STREAMS_PER_GPU; s++) {
        int streamStartSymbol = s * singleSymbolsPerStream;
        int streamEndSymbol = streamStartSymbol + singleSymbolsPerStream;
        if (s == STREAMS_PER_GPU - 1) streamEndSymbol += singleRemainingSymbols;
        ...
        gpuCheck(cudaMemcpyAsync(&batch_Y[streamStartSymbol * ELEMENTS_PER_SYMBOL],
                 ...
                 cudaMemcpyDeviceToHost, single_streams[s]), "copy Y single stream");
    }
    
    // Synchronize all D2H transfers
    for (int s = 0; s < STREAMS_PER_GPU; s++) {
        gpuCheck(cudaStreamSynchronize(single_streams[s]), "sync single d2h stream");
    }
    ...
    singleGpuResults.num_streams_used = STREAMS_PER_GPU;
    ...
    for (int dev = 0; dev < deviceCount; dev++) {
        ...
        int gpuSymbols = endSymbol - startSymbol;
        int gpuElements = gpuSymbols * ELEMENTS_PER_SYMBOL;
    ...
    printf("✅ Single GPU: processed entire dataset with %d streams\n", STREAMS_PER_GPU);
