...
main (void)
{
  ...
  printf("=== Simultaneous Multi-GPU Kernel Launch Analysis ===\n");
  printf("Found %d CUDA device(s)\n", deviceCount);
  printf("Data size: %d MB per GPU\n", (FULL_DATA_SIZE * sizeof(int) * 3) / (1024 * 1024));
  ...
  // ==============================================
  // SEQUENTIAL MULTI-GPU (for comparison)
  // ==============================================
  printf("=== Sequential Multi-GPU Launch ===\n");
  
  HANDLE_ERROR(cudaSetDevice(0));
  HANDLE_ERROR(cudaEventRecord(global_start, 0));
  
  // Sequential approach - launch kernels one GPU at a time
  for (int dev = 0; dev < deviceCount; dev++) {
    HANDLE_ERROR(cudaSetDevice(dev));
  ...
  // ==============================================
  // SIMULTANEOUS MULTI-GPU LAUNCH WITH DETAILED TIMING
  // ==============================================
  printf("=== Simultaneous Multi-GPU Launch with Detailed Timing ===\n");
  
  // Pre-copy all data to GPUs first to isolate kernel launch overhead
  printf("Pre-copying data to all GPUs...\n");
  HANDLE_ERROR(cudaSetDevice(0));
  ...
  for (int dev = 0; dev < deviceCount; dev++) {
    ...
    // Pre-copy all data for this GPU
    for (int chunk = startChunk; chunk < endChunk; chunk += 2) {
      int dataOffset1 = chunk * N;
      int dataOffset2 = (chunk + 1) * N;
      ...
      if (chunk < endChunk) {
        HANDLE_ERROR(cudaMemcpy(dev_a[dev * 2], host_a + dataOffset1, 
                   N * sizeof(int), cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy(dev_b[dev * 2], host_b + dataOffset1, 
                   N * sizeof(int), cudaMemcpyHostToDevice));
      }
      
      // Copy data for stream 1
      if (chunk + 1 < endChunk) {
        HANDLE_ERROR(cudaMemcpy(dev_a[dev * 2 + 1], host_a + dataOffset2, 
                   N * sizeof(int), cudaMemcpyHostToDevice));
        HANDLE_ERROR(cudaMemcpy(dev_b[dev * 2 + 1], host_b + dataOffset2, 
                   N * sizeof(int), cudaMemcpyHostToDevice));
      }
    }
  ...
  printf("\nMeasuring pure kernel launch overhead and execution...\n");
  ...
  // Launch ALL kernels across ALL GPUs without any memory transfers
  for (int dev = 0; dev < deviceCount; dev++) {
    HANDLE_ERROR(cudaSetDevice(dev));
    ...
    printf("  GPU %d: launching %d kernels\n", dev, (endChunk - startChunk + 1) / 2 * 2);
    
    // Launch kernels only (data already on GPU)
    for (int chunk = startChunk; chunk < endChunk; chunk += 2) {
  ...
  printf("Pure kernel launch time: %6.3f ms\n", pureKernelLaunchTime);
  ...
  for (int dev = 0; dev < deviceCount; dev++) {
    ...
    for (int chunk = startChunk; chunk < endChunk; chunk += 2) {
      ...
      if (chunk < endChunk) {
        HANDLE_ERROR(cudaMemcpy(host_c + dataOffset1, dev_c[dev * 2], 
                   N * sizeof(int), cudaMemcpyDeviceToHost));
      }
      
      if (chunk + 1 < endChunk) {
        HANDLE_ERROR(cudaMemcpy(host_c + dataOffset2, dev_c[dev * 2 + 1], 
                   N * sizeof(int), cudaMemcpyDeviceToHost));
      }
    }
    HANDLE_ERROR(cudaDeviceSynchronize());
  ...
  printf("Multi-GPU scaling:            %s and efficiency is %d",
         efficiency > 85 ? "EXCELLENT" : efficiency > 70 ? "GOOD" : "LIMITED", efficiency);
